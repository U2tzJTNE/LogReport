...
import com.u2tzjtne.logrepoter.core.save.BaseSaver;
...
public class CrashWriter extends BaseSaver {
     ...
     * 。系统默认异常处理
     */
    private static final Thread.UncaughtExceptionHandler S_DEFAULT_HANDLER = Thread.getDefaultUncaughtExceptionHandler();
        ...
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                synchronized (BaseSaver.class) {
                    ...
                    if (S_DEFAULT_HANDLER != null) {
                        S_DEFAULT_HANDLER.uncaughtException(thread, ex);
                    }
                }
            }
        });
